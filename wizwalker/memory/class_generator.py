...
class InheritanceTree:
    def __init__(self, class_type: "wizwalker.memory.type_tree.Type", class_generator: "ClassGenerator"):
        # base type
    ...
    async def handle_static_container(
        ...
        if return_name.startswith("class ") or return_name.startswith("struct "):
            ...
            lines = [f'return await self.read_enum_from_offset({offset}, {clean_return})']

        elif return_name in wizwalker.type_format_dict.keys():
            lines = [f'return await self.read_value_from_offset({offset}, "{return_name}")']
            ...
            match return_name:
                case "__int64":
                    lines = [f'return await self.read_value_from_offset({offset}, "long long")']
                    arrow_return = "int"
                ...
                case "std::wstring":
                    lines = [f"return await self.read_wide_string_from_offset({offset})"]
                    arrow_return = "str"
                ...
                case "bui4" | "bui6" | "bui3" | "bui7" | "bui5":
                    lines = [f'return await self.read_value_from_offset({offset}, "unsigned int")']

                case _:
                    raise RuntimeError(f"Unhandled static type {return_name}")

        return self.get_method_string(clean_property_name, lines, return_type=arrow_return)
    ...
    async def handle_class(
        ...
        for prop in await property_list.properties():
            ...
            match container_name:
                case "Static":
                    # TODO: handle Point and Rect special cases
                    if return_type_name.startswith("class ") or return_type_name.startswith("struct "):
                        ...
                        lines.append(f'return await self.read_enum_from_offset({offset}, {cleaned_return_name})')

                    elif return_type_name in wizwalker.type_format_dict.keys():
                        lines.append(f'return await self.read_value_from_offset({offset}, "{return_type_name}")')
                        ...
                        match return_type_name:
                            case "__int64":
                                lines.append(f'return await self.read_value_from_offset({offset}, "long long")')
                            ...
                            case "std::string":
                                lines.append(f"return await self.read_string_from_offset({offset})")
                                arrowed_return_name = "str"

                            case "std::wstring":
                                lines.append(f"return await self.read_wide_string_from_offset({offset})")
                                arrowed_return_name = "str"
                            ...
                            case "bui4" | "bui6" | "bui3" | "bui7" | "bui5":
                                lines.append(f'return await self.read_value_from_offset({offset}, "unsigned int")')

                            case _:
                                raise RuntimeError(f"Unhandled static type {return_type_name}")
                ...
                case "Vector":
                    ...
                    # dynamic
                        ...
                        if return_type_is_pointer:
                            raise RuntimeError("Attempting to write inline vector for pointer vector")
                        pass
