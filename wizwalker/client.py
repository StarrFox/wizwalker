...
class Client:
    ...
    def zone_name(self) -> Optional[str]:
        ...
        """
        client_zone = self.client_object.client_zone()

        if client_zone is not None:
            # noinspection PyBroadException
            try:
                return client_zone.zone_name()
            except Exception:
                return None

        return None

    def get_base_entity_list(self):
        """
        List of WizClientObjects currently loaded
        """
        root_client = self.client_object.parent()
        return root_client.children()

    def get_base_entities_with_name(self, name: str):
        """
        ...
        def _pred(entity):
            object_template = entity.object_template()
            return object_template.object_name() == name

        return self.get_base_entities_with_predicate(_pred)

    # TODO: add example
    def get_base_entities_with_predicate(self, predicate: Callable):
        """
        ...
        for entity in self.get_base_entity_list():
            if predicate(entity):
                entities.append(entity)

        return entities

    def get_world_view_window(self):
        """
        ...
        pos = self.root_window.get_windows_with_name("WorldView")
    ...
    def get_template_ids(self) -> dict:
        ...
        self._template_ids = self.cache_handler.get_template_ids()
        return self._template_ids
    ...
    def in_battle(self) -> bool:
        ...
        """
        try:
            duel_phase = self.duel.duel_phase()
    ...
    def is_loading(self) -> bool:
        ...
        return self.hook_handler.read_typed(self._is_loading_addr, "bool")

    def is_in_dialog(self) -> bool:
        """
        If the client is in dialog
        """
        world_view = self.get_world_view_window()
        for child in world_view.children():
            # TODO: check if we also need to check for wndDialogMain child
            if (child_name := child.name()) == "NPCServicesWin":
                return True
    ...
    def is_in_npc_range(self) -> bool:
        """
        If the client is within an npc interact range
        """
        world_view = self.get_world_view_window()
        for child in world_view.children():
            if child.name() == "NPCRangeWin":
                return True

        return False

    def backpack_space(self) -> tuple:
        """
        This client's backpack space used and max
        must be on inventory page to use
        """
        maybe_space_window = self.root_window.get_windows_with_name(
            "inventorySpace"
        )
        ...
        text = maybe_space_window[0].maybe_text()
        text = text.replace("<center>", "")
        used, total = text.split("/")
        return int(used), int(total)

    def wait_for_zone_change(
    ...
    def current_energy(self) -> int:
        ...
        text = maybe_energy_text[0].maybe_text()
        text = text.replace("<center>", "")
        text = text.replace("</center>", "")
        return int(text)
    ...
    def send_key(self, key: Keycode, seconds: float = 0):
        """
        Send a key
        ...
        """
        utils.timed_send_key(self.window_handle, key, seconds)
    ...
    def goto(self, x: float, y: float):
        ...
        Args:
            ...
            y: Y to move to
        """
        current_xyz = self.body.position()
        # (40 / 100) + 1 = 1.4
        speed_multiplier = (self.client_object.speed_multiplier() / 100) + 1
        ...
        self.body.write_yaw(yaw)
        utils.timed_send_key(self.window_handle, Keycode.W, move_seconds)

    def teleport(self, xyz: XYZ, yaw: float = None, *, move_after: bool = True):
        """
        Teleport the client
        ...
        """
        self.patch_infinite_loading()

        self.body.write_position(xyz)

        if move_after:
            self.send_key(Keycode.D, 0.1)

        if yaw is not None:
            self.body.write_yaw(yaw)

    def patch_infinite_loading(self):
        """
        ...
        je_addr = self.hook_handler.pattern_scan(
        ...
        self._infinite_patch_og_info = (
            je_addr,
            self.hook_handler.read_bytes(je_addr, 6),
        )

        jmp_offset = self.hook_handler.read_typed(je_addr + 2, "unsigned int")
